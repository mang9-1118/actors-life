...
`castingBookmarkletHref(appUrl, appKey)`이 `javascript:void((함수)("앱주소","키"))` 문자열을
만든다. 오디션 지원 탭의 `[오디션 가져오기]` 버튼이 그 주소를 달고 있고, 사용자가 북마크바로
끌어다 놓는다. 약 3KB.

- **`readCastingNotice()`는 자기완결적이어야 한다.** 함수를 문자열화해서 내보내므로
  모듈 스코프를 참조하면 번들러가 이름을 바꿔 공고 페이지에서 터진다. `//` 주석도 두지
  않는다 (한 줄로 직렬화되므로).
- 앱 주소와 접근 키가 렌더 시점에 박힌다. 즉 **배포된 앱에서 끌어다 놓은 북마클릿은 배포된
  앱으로, 로컬 빌드에서 끌어다 놓은 것은 로컬로 돌아온다.** 배포 주소나 접근 키가 바뀌면
  다시 등록해야 한다.
...
### 값은 서버를 경유해 열어둔 앱 탭으로 간다

브라우저는 공고 사이트의 페이지가 무관한 탭을 건드리지 못하게 한다. 막힌 길을 먼저 적어둔다.

- `window.open(url, '이름')`의 **이름은 여는 쪽의 브라우징 컨텍스트 그룹 안에서만** 찾는다.
  그래서 ① 사용자가 직접 열어둔 앱 탭은 절대 찾지 못하고, ② 필름메이커스에서 만든 앱 탭과
  플필에서 만든 앱 탭이 서로 다른 그룹이라 각자 생긴다. (한동안 이 방식이었다.)
- 앱 오리진의 숨은 iframe을 심어 `postMessage`로 넘기는 방법도 안 된다. 크롬의 저장소
  분리(storage partitioning)로 **서드파티 iframe의 `localStorage`/`BroadcastChannel`이
  최상위 사이트별로 갈라져** 실제 앱 탭과 통하지 않는다.
- 남는 길은 서버 경유, 클립보드, 확장 프로그램뿐이다. **서버 경유를 택했다.**

`api/casting-inbox.ts`가 그 우편함이다. 북마클릿이 값을 POST하고, 열어둔 앱 탭이
`takePendingNotice()`로 가져간다. **새 탭이 생기지 않고 공고 페이지도 그대로다.**

- 저장은 Vercel Blob (`casting-inbox/<시각>.json`, `access: 'private'`) — `api/backup.ts`와
  같은 방식이다. **읽으면 비운다.** 여러 개가 밀려 있으면 가장 최근 것을 주고 전부 지운다.
  30분이 지난 것은 버린다.
- POST는 공고 사이트 오리진에서 오므로 **CORS가 필요하다.** 허용 목록은 필름메이커스·플필뿐.
  JSON 본문 + `x-app-key` 때문에 프리플라이트가 뜨니 `OPTIONS`도 처리한다.
- 앱은 **탭을 다시 볼 때**(`visibilitychange`·`focus`) 확인하고, 화면에 떠 있는 동안 30초마다
  한 번 더 본다. 이 확인 한 번이 Blob 목록 조회 한 번이라 더 촘촘히 돌리면 요청만 낭비된다.
  실패하면 조용히 넘어간다 — 타이머마다 경고를 띄우면 소음이다.
- **접근 키가 북마클릿 주소에 박힌다.** 개인 기기의 북마크에 들어가는 것이고 공고 페이지가
  북마클릿 소스를 읽을 수는 없지만, 북마클릿은 그 페이지의 JS 컨텍스트에서 돌기 때문에
  페이지가 `window.fetch`를 미리 바꿔놨다면 키를 볼 수 있다. 정상 사이트에서는 무리한 걱정이나
  알고는 있을 것.
- **실패하면 탭을 열지 않고 이유를 띄운다** (빨간 알림, 6초). `401 · 접근 권한이 없습니다.`
  처럼 상태 코드와 서버 메시지가 함께 나온다. 예전에는 앱 탭을 여는 대체 경로였는데, 탭이
  하나씩 늘면서 "어느 탭에 무엇이 저장됐는지" 알 수 없게 되는 편이 더 나빴다. 실패는 원인을
  고치고 다시 누르면 된다.
- 해시 경로(`#/train/audition-apply?import=...`)는 **예전에 등록해둔 북마클릿을 위해 남겨둔다.**
  새 북마클릿은 쓰지 않는다.

**새 탭이 열린다면 그건 우편함에 닿지 못했다는 신호가 아니라, 우편함이 생기기 전의 북마클릿을
그대로 쓰고 있다는 신호다.** 새 북마클릿은 어떤 경우에도 탭을 열지 않는다.
...
- **우편함의 정상 경로는 배포 환경에서만 확인할 수 있다.** Blob 자격증명이 로컬에 없어서
  `put`/`list`/`get`이 500으로 떨어진다. 인증·검증·CORS·메서드 분기와 그 500 처리까지는
  핸들러를 직접 호출해 확인했지만, **실제로 값이 오가는 것은 배포 후 눌러봐야 한다.**
- **브라우저에서 북마클릿을 실제로 등록해 눌러본 적이 없다.** jsdom으로 파싱과 POST는
  검증했지만, 북마크바 드래그 → 클릭 → 앱 탭에 채워지는 흐름은 사람이 한 번 해봐야 한다.
