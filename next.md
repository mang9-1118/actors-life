...
- 공고 상세가 아닌 페이지에서 누르면 `alert`로 알린다. 사이트별로 다른 문구가 나오도록
  필름메이커스는 JSON-LD `JobPosting`이 있는지, 플필은 `castingEndDate`가 있는지로 상세
  페이지를 판별한다 (플필 목록 페이지에도 `pageProps.data`는 있다). 앱 안에서 눌러도
  안내가 뜨고, 그게 이 버튼의 사용법 힌트 역할을 한다.
...
| 지원처 | `필름메이커스` 고정 | `플필` 고정 |
| 공고 링크 | 쿼리스트링 뺀 주소 (북마클릿은 `location.origin + pathname`, 서버는 직접 정규화) | 같음 |

플필은 북마클릿과 서버가 같은 필드를 읽으므로 어느 경로로 가져와도 결과가 같다 (실제 공고로
양쪽 다 확인).
