...
오디션 지원 탭의 "주소 불러오기"가 지금은 **필름메이커스만** 지원한다. 플필도 함께
지원하는 게 목표. 플필 어댑터는 한 번 만들어서 동작까지 확인했고, 범위를 원래대로
되돌리면서 제거했다.

### 플필 코드 복원

`api/casting.ts` 한 파일에만 있었다. 제거 직전 버전에서 그대로 꺼내 쓸 수 있다.

```
git show 30fcacb:api/casting.ts
```

거기서 가져올 것 세 개: `plfilNoticeData()`, `stringField()`, `PLFIL` 상수.
그리고 `const SITES: CastingSite[] = [FILMMAKERS, PLFIL]`로 배열에 추가하면 끝.
안내 문구(`SUPPORTED_HINT`)는 배열에서 자동 생성되므로 따로 손댈 필요 없다.

프론트엔드·타입·스토어는 사이트와 무관하게 이미 일반화되어 있어 수정할 게 없다.
