...
interface AnalysisState {
  ...
  addConceptSummary: (input: Omit<ConceptSummary, 'id' | 'createdAt'>) => void
  updateConceptSummary: (id: string, patch: Omit<ConceptSummary, 'id' | 'createdAt'>) => void
...
export const useAnalysisStore = create<AnalysisState>()(
  persist(
    (set) => ({
      ...
      updateConceptSummary: (id, patch) =>
        set((state) => ({
          conceptSummaries: state.conceptSummaries.map((s) =>
            s.id === id ? { ...s, ...patch } : s,
          ),
        })),
